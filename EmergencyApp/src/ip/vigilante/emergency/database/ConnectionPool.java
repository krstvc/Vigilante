package ip.vigilante.emergency.database;
...
public class ConnectionPool {
	...
	static {
		ResourceBundle bundle = PropertyResourceBundle.getBundle("ip.vigilante.emergency.database.ConnectionPool");
		...
		try {
			connectionPool = new ConnectionPool(jdbcURL, username, password,
					preconnectCount, maxIdleConnections, maxConnections);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
  }

	protected ConnectionPool(String aJdbcURL, String aUsername, String aPassword, 
			int aPreconnectCount, int aMaxIdleConnections, int aMaxConnections)
