...
public class EmergencyCallDAO {
	...
	private static final String SQL_INSERT_CALL = "insert into `ip`.`emergency_call`"
			...
			+ " values (?, ?, ?, ?, ?, ?, ?, ?);";
	private static final String SQL_UPDATE_CALL = "update `ip`.`emergency_call`"
			+ " set `report_count` = ?"
			+ " where `id` = ?;";
	...
	public static boolean insertCall(EmergencyCall call) {
		...
		String time = call.getTime() != null ? df.format(call.getTime()) : null;
	...
	public static boolean updateCall(EmergencyCall call) {
		...
		try {
			conn = connPool.checkOut();
			PreparedStatement ps = DAOUtil.prepareStatement(conn, SQL_UPDATE_CALL, false, values);
